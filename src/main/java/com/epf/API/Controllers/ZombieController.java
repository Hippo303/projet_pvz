...
public class ZombieController {
    ...
    public ResponseEntity<DTOZombie> updateZombie(@PathVariable ("id") Integer id, @RequestBody DTOZombie dtoZombie) {
        Zombie zombie = mapperZombie.toZombieEntity(dtoZombie);
        Zombie updtatezombie = zombieServices.updateZombie(zombie, id);
        if (updtatezombie!=null) {
            DTOZombie updtatedtozombie = mapperZombie.toDTOZombie(updtatezombie);
            return new ResponseEntity<>(updtatedtozombie,HttpStatus.OK);
        }else {return new ResponseEntity<>(HttpStatus.NOT_FOUND);}
